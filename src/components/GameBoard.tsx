...
import type { BoardCell, BoardPreviewMode, Position } from '../types/game';
    ...
    <section className="pixel-card w-full min-w-0 max-w-[700px] p-3 sm:p-4">
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm font-semibold text-slate-500">코드를 정확히 실행하면 지나간 칸만 색칠됩니다.</p>
        </div>
      ...
      <div className="mx-auto w-full">
        <div className="relative aspect-square rounded-2xl border-[3px] border-slate-900 bg-slate-900 p-1.5 shadow-[0_10px_0_rgba(15,23,42,0.16),0_24px_44px_rgba(15,23,42,0.16)] sm:border-4">
          <div
              ...
              row.map((cell, colIndex) => {
                ...
                return (
                  <div
                    ...
                    aria-label={`${rowIndex}행 ${colIndex}열 ${cell.type}`}
                  >
          ...
          {columnCount > 0 && rowCount > 0 ? (
            <div
              className={[
                'pointer-events-none absolute left-1.5 top-1.5 flex items-center justify-center',
                'transition-[transform] duration-300 ease-out',
                isAnimating ? 'scale-105' : '',
              ].join(' ')}
              style={{
                width: `calc((100% - 0.75rem) / ${columnCount})`,
                height: `calc((100% - 0.75rem) / ${rowCount})`,
